...
static int do_receiver_part(t_direccion *dir_local, t_direccion *dir_remota) {
  ...
  do {
    char buf[8192];//8192
    //fprintf(stderr,"Esperamos recibir %i bytes.\n", bs); 
    len1 = t_receive(cx1, buf, bs, &flags);
    j++;  
    if (len1 < 0)  fprintf(stderr,"len1 = %i\n",len1);
    else recibidos += len1;
    if ((j % 500) == 0)  fprintf(stderr,"Rec %i bytes.\n", recibidos);  
    ...
    if (flags != 0) fprintf(stderr,"Los flags despues de recv estan a %i.\n",flags);
    len2 = t_send(cx2, buf, len1, &flags);
    if (flags != 0) fprintf(stderr,"Los flags despues de recv estan a %i.\n",flags);
    i++;  
    if (len2 < 0)  fprintf(stderr,"len2 = %i\n",len2);
    else enviados += len2;
    if ((i % 500) == 0) fprintf(stderr,"Env %i bytes.\n",enviados);  
  ...
  fprintf(stderr,"Antes del t_flush(cx2)!!\n"); 
  t_flush(cx2);
  fprintf(stderr,"Paso el t_flush(cx2)!!\n"); 
  if ((flags & CLOSE) == CLOSE){
    
    fprintf(stderr,"t_disconnect(cx1) devuelve: %i.\n",t_disconnect(cx1));
    fprintf(stderr,"t_disconnect(cx2) devuelve: %i\n",t_disconnect(cx2));
  }
    fprintf(stderr,"Se han enviado %i bytes y se han recibido %i bytes\n", enviados,recibidos);
  return EXOK;
}
...
static int do_sender_part(t_direccion *dir_local, const t_direccion *dir_remota, size_t len) {
  ...
  //
  //sleep(1);   //DESCOMENTAR ESTE SLEEP SI SE HACE ESTABLECIMIENTO DE CONEXION CON: ida-vuelta-ida
  //

  cx2 = t_connect(&dir_remota2,dir_local);
  if (cx2 < 0) {
    t_disconnect(cx1);
    return cx2;
  }
  ...
  do {
    ...
    k++;
    // fprintf(stderr, "   Quedan por mandar %zd Bytes   \n", len);
    if (bs >= len) {
      fprintf(stderr,"numerode ejecuciones %i\n",k);
      flags |= CLOSE;
      bs = len;
    }
      //fprintf(stderr,"Esperamos enviar %i bytes.\n",bs);
    len1 = t_send(cx1, data, bs, &flags);
    i++;  
    if (flags != 0) fprintf(stderr,"Los flags depues de send estan a %i.\n",flags);
    if (len1 < 0)  fprintf(stderr,"len1 = %i\n",len1);
    else enviados += len1;
    if ((i % 500) == 0) fprintf(stderr,"Env %i bytes.\n",enviados);  
    if (len1 < 0) {
    ...
    j++;  
    if (flags != 0) fprintf(stderr,"Los flags depues de recv estan a %i.\n",flags);
    if (len2 < 0)  fprintf(stderr,"len2 = %i\n",len2);
    else recibidos += len2;  
    if ((j % 500) == 0)  fprintf(stderr,"Rec %i bytes.\n", recibidos);  
    if (len2 < 0 && len2 != EXCLOSE && len2 != EXDISC) {
...
int main(int argc, char *argv[]) {
  bool listen = false;
  t_direccion dir_origen = {IN6ADDR_ANY_INIT, 0}, dir_destino = {0, 0};
  int error;
  int opt;
